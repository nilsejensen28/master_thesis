...
In this chapter, we will discuss where the \Tarski\ problem lies in the complexity landscape of \TFNP. We will start by giving an overview of the known results and explain how they can be combined to obtain that \Tarski\ lies in \EOPL. We will then discuss why it is challenging to chain these results together to obtain a reduction from \Tarski\ to the \EndOfPotentialLine\ problem, which gives us some insight into why \Tarski\ lies in \EOPL. This will motivate our approach in the following chapters to circumvent these difficulties.
...
Previous work~\sidecite{buss_propositional_2012} showed that many-to-one reductions and Turing-reduction onto \PPAD\ are equivalent. In particular this means that $\P^{\PPAD} = \PPAD$, and that \Tarski\ lies in \PPAD{}. This proof does not yield a reduction and is based on a proof-complexity argument
...
A further notable collapse is the result $\PLS \medcap \PPAD = \EOPL$, which was only recently shown in \sidecite{goos_further_2022}. This means that \Tarski\ lies in \EOPL.
