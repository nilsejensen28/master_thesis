...
\begin{itemize}
    ...
    \item Papadimitriou proved that \Brouwer\ is \PPAD-complete by reducing \Brouwer\ to \Sperner\ in \sidecite{papadimitriou_complexity_1994-1}. Hence by reducing to \Brouwer, we introduce continuity into the problem, which is not necessary.
\end{itemize}
...
Next we introduce the problem \Sperner, which is a total search problem, that was introduced and shown to be \PPAD-complete in \sidecite{papadimitriou_complexity_1994-1}. We introduce the problem for a general lattice $L = N_1 \times \dots \times N_d$, such that $N_i \leq 2^n$.
...
\marginnote[-15mm]{We define Sperner on a integer euclidian lattice for convenience, but it can be defined on any simplicial decomposition of the space.}
