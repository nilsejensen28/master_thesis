...
In the previous Section, we introduced \textit{Sperner's Lemma} for an large outer simplex. We want to make construct a \TFNP\ problem, which captures the finding of the subsimplex containing all colors. The challenge here is that encoding the structure of this simplex is not straightforward, and hence defining circuits which encode the setting of the problem will be difficult. Furthermore our final goal is to reduce \Tarskistar\ to the problem of fidnding a subsimplex containing all colors. The setting we are given in this case is not a simplex but an integer lattice.

For these reasons, we want to apply \textit{Sperner's Lemma} to an integer lattice. We proceed as follows: We take the $d$-dimensional lattice $L = [N_1] \times \dots \times [N_d]$, we subdivide each cell into simplices\sidenote{How this we do this is not relevant in this chapter but will be discussed in the next chapter.}. Now we need a technical lemma, which states that the simplicial complex we obtain can be deformend into a large outer simplex.
...
\begin{proof}
	...
	\begin{align*}
		D(y) = \gamma \cdot D(x)
	\end{align*}
	This contruction has some key properties. Most importantly, segments (edges of the simplical decomposition) are mapped to segments, and simplices are mapped to simplices. This means that the simplicial decomposition of the lattice is mapped to a simplicial decomposition of the simplex. This means that we can apply \textit{Sperner's Lemma} to the simplicial decomposition of the lattice. This concludes the proof.
\end{proof}
...
In this section we will discuss the reduction of \Sperner\ to \EndOfLine. This reduction was first constructed in the three-dimensional case in \sidecite{papadimitriou_complexity_1994-1}. The same paper also gives the idea of the generalization to the $d$-dimensional case. In the following we will give the full construction of the reduction. This is important as we will use this reduction to argue as to why \Tarskistar\ is in \EOPL.

\begin{theorem}[\Sperner\ is in \PPAD]
	\Sperner\ reduces to \EndOfLine.
\end{theorem}

The idea of the reduction is the following. We want to want to find a cell which contains all colors: assume that we start at a cell with all but one color $d$. Then move to the neighbooring cell though a face containing all colors but $d$. Now either this cell contains the color $d$ in which case we are finished, or we have a second face containing all colors but $d$. We can repeat this process until we find a cell containing all colors.


Now there are two problems we need to discuss. First once again using cell leads to some difficulty as there could be more than two faces for a given cell, which could contain all colors but one. We will solve this problem once again by considering a simplicial decomposition of the lattice. Second, we need to define a designated source. In order to do this we will slightly expand the simplicial complex. We are now ready for the formal proof, we recommend the reader to follow along with the construction in \cref{fig:sperner_eol_reduction}.
...
\begin{proof}
	Formally we will proceed by induction over the dimensions of the lattice. First we discuss the base-case. In the one dimensional case, we have a lattice $L = [N]$. We color the lattice with colors $\set{0, 1}$. Now for every segment $s = [x, x+1]$, of which there are $N-1$ which we number from left to right of the latice we define the circuits $S, P : [N-1] \rightarrow [N-1]$ as follows:
	\begin{align*}
		S(x) & = \begin{cases}
			         x + 1 & \text{if $c(x+1) = 0$} \\
			         x     & \text{else}
	...
	Now we are ready to prooceed with the induction. Assume the theorem is true for all dimensions $1, \dots, d-1$. We will prove it for dimension $d$.
	...
	\begin{case}{1}
		If $x$ has a vertex with every color in $\set{0, \dots, d-1}$, but no vertex colored $d$, then $x$ has two faces colored with all colors but $d$. One of these faces is oriented positively and one of the faces oriented negatively \boxmarginnote{See \cref{sec:orientation_of_simplex} for a detailed dicussion of how we define these orientations}. Now define $S(x)$ to be the simplex obtained by moving from $x$ to the positively oriented face. If this face contains a vertex colored $d$, then we define $P(x)$ to be the simplex obtained by moving to the negatively oriented face, and $P(x)$ the simplex obtained by moving through the negatively oriented face.
	\end{case}
	\begin{case}{2}
		If $x$ is a simplex with all colors, then look at the face spanned by the vertices colored $\set{0, \dots, d-1}$. If this face is oriented positively, then define $S(x)$ to be the simplex obtained by moving through this face and $P(x) = x$. If this face is oriented negatively, then define $P(x)$ to be the simplex obtained by moving through this face and $S(x) = x$. Notice that these are the sources/sinks of the circuit, and the solutions to the sperner instances.
	\end{case}
	\begin{case}{3}
		In all other cases we define $S(x) = P(x) = x$.
	\end{case}
	Notice that we can compute $S$ and $P$ in polynomial time with respect to $d$. We still need to define a distinguished source. Notice that finding a distinguished source can be seen as finding a face colored with all colors $\set{0, \dots, d-1}$ on the face spanned by the vertices $v_0, \dots, v_{d-1}$. We can do this by solving a $d-1$ dimensional \Sperner\ instance on this face. By induction hypothesis this can be reduced to an \EndOfLine\ instance. We can now define the distinguished source as the source of this \EndOfLine\ instance, and slightly modify the circuit $S$ and $P$ to combine this \EndOfLine\ instance with the circuits $S$ and $P$ we have constructed above. This concludes the proof.
\end{proof}
