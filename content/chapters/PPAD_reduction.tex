...
\begin{proof}
    ...
    \marginnote{We do not actually need to check these points, it suffice to have the algorithm stop if at any point it notices that $f$ leaves $S_x$, as this would lead to a violation of monoticity.}
...
Next we introduce the problem \Sperner, which is a total search problem, that was introduced and shown to be \PPAD-complete in \sidecite{papadimitriou_complexity_1994-1}. We introduce the problem for a general latice $L = N_1 \times \dots \times N_d$, such that $N_i \leq 2^n$.
...
\marginnote[-15mm]{We define Sperner on a euclidian latice for convenience, but it can be defined on any simplicial decomposition of the space.}
