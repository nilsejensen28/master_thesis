...
In chapter XX introduced \textit{Sperner's Lemma} for an integer lattice. This enables us to find a region of a colored lattice that contains all colors under certain conditions. Instead of a subsimplex, we look for a \define{cell}\sidenote{By cell, we mean a unit hypercube of the integer lattice} of the lattice, which contains all colors.
