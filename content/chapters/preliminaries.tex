...
\begin{definition}[Function \NP\ (\FNP)]
	We say that a relation $R\subset \binstr \times \binstr$ is in \FNP\ if it is \define{polynomially balanced}, i.e.\ there exists a polynomial $p$ such that for every $(I, s) \in R$ implies $\length{y} \leq p(\length{x})$.
...
\begin{definition}[Turing Reduction]
	For two problems $R, S \in \TFNP$, we say that $R$ \emph{Turing reduces} to $S$ if a polynomial-time oracle Turing machine that solves $R$ given access to an oracle\boxmarginnote{An \define{oracle} is a black-box which solves $S$.} for $S$ exists.
\end{definition}
...
Additionally to this syntactic validation, we can construct \TFNP-instances by adding \define{violations} to the solution space. For example, if we are interested in finding the global minima of convex functions, we can construct a total search problem by:
\begin{enumerate}
...
Now that we have introduced \PPAD, we introduce a slight variation which we call \PPADS, first introduced in \sidecite{papadimitriou_graph-theoretic_1990} as \textbf{PSK} and formalized in \sidecite{beame_relative_1998}. The cannonical problem which gives rise to \PPADS\ is \textsc{Sink-of-Line}, which is the identical to \EndOfLine\ except that only sinks --- and not further sources --- are solutions. We introduce it formally:

\problem{Sink-of-Line (\textsc{SoL})}{
Boolean circuit $S, P : \bitstr^n \rightarrow \bitstr^n$ such that $P(0^n) = 0^n \neq S(0^n)$ ($0^n$ is a source.)
}{
An $x \in \bitstr^n$ such that:
\begin{itemize}
	\item $P(S(x)) \neq x$ ($x$ is a sink)
\end{itemize}
}

This then gives rise to \PPADS\ as previously:
\begin{definition}[\PPAD]
	The class \PPADS\ is the set of all \TFNP\ problems that reduce to \textsc{Sink-of-Line}.
\end{definition}

We note that Papadimitriou thought that $\PPAD{} = \PPADS{}$ \cite{papadimitriou_graph-theoretic_1990}. This turns out \emph{not} to be true, as we in fact have $\PPADS{} \subsetneqq \PPAD{}$~\cite{beame_relative_1998}.
...
The \Tarski-problem originates from Tarski's fixed point Theorem, introduced in \sidecite{tarski_lattice-theoretical_1955}. In our setting the Theorem states the following:
...
\end{proof}
This means that in this thesis, we can consider the simplified version of the \Tarski\ problem\marginnote{Also note that given a circuit that computes $f$, we can construct a circuit with at most $\BigO{\poly{d}}$ additional gates which computes $\ftilde$.}, where for every $x \in [2^n]^d$, we have $\norminf{x - f(x)} \leq 1$, which we will implicitly assume from now on.
...
A further notable collapse is the result $\PLS \medcap \PPAD = \EOPL$, which was only recently shown in \sidecite{goos_further_2022}. This means that \Tarski\ lies in \EOPL. A question that then arises, and which this thesis will attempt to answer, is whether we can construct an explicit reduction of \Tarski\ to \EndOfPotentialLine.
...
\problem{UniqueTarski}{A boolean circuit $f : {[N]}^d \rightarrow {[N]}^d$ such that $f$ is monotone and there is a unique fixpoint $x$}{The unique fixpoint $x$ of $f$.}

A very notable recent result, is that the query complexity of \UniqueTarski\ is equal to the query complexity of \Tarski\ \sidecite{chen_reducing_2023}. We also introduce an even stronger uniqueness variant of \Tarski next:

\problem{SuperUniqueTarski}{A boolean circuit $f : {[N]}^d \rightarrow {[N]}^d$ such that $f$ is monotone and when fixing a subset of coordinates the induce function $\ftilde$ has a unique fixpoint.}{The unique fixpoint $x$ of $f$.}
