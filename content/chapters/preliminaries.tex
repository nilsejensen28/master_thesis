...
TODO: Write about order preserving/monotone functions.
...
\begin{definition}[Progress point]
    \marginnote[0mm]{The smallest and largest point of a lattice are always progress points.}
...
We now want to discuss the lower bounds for solving \Tarski\ instances. The best known lower bounds for \Tarski\ are given by \sidecite{etessami_tarskis_2020}. They showed that in the black-box model, where the only way to access the function $f$ is by querying it, solving a $d$-dimensional \Tarski\ requieres solving at least $\Omega(\log^{N})$ $1$ dimensional \Tarski\ instances, which are as difficult as binary search,hence this means that solving a $d$-dimensional \Tarski\ instance requieres at least $\Omega(\log^{2}{N})$ queries. This means that the upper and lower bounds are equal in the 2 dimensional case, but in all other cases there remains a gap. In particular the best known lower bound for solving \Tarski\ does not depend on the dimension $d$, which seems somewhat unexpected.
