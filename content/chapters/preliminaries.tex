...
The name of the \Tarski\ problem originates from Tarski's fixed point Theorem, first introduced in \sidecite{tarski_lattice-theoretical_1955}. It goes as follows:
...
\end{proof}
This means that in this thesis, we can consider the simplified version of the \Tarski\ problem, where for every $x \in L$, we have $\norminf{x - f(x)} \leq 1$, which we will implicitly assume from now on.
...
\end{remark}
Now, by noting that by starting at the point $\mathbf{0} = 0^d$ and iterating the function $f$, we will eventually reach a fixed point, we can construct an iterative algorithm for solving \Tarski, described in \cref{alg:iterative_tarski_solver}.
...
Next, we describe a more advanced algorithm for solving \Tarski\ instances and also give an alternative presentation and simplified proof of its correctness. The algorithm we will present is due to \sidecite{dang_computations_2020}. The following notation aim to make the argument as clear as possible. For a given complete lattice $L = [N_1] \times \cdots \times [N_d]$ and some dimension $x \in L$ we define the following sublattices:
\begin{align*}
...
A simple analysis shows that this algorithm runs in $\BigO{\log^{d}{N}}$ for $L = [N]^d$. It was conjectured by Etessami et.\ al.\ that this is an optimal algorithm for \Tarski\ \sidecite{etessami_tarskis_2020}. This turned out not to be true, as a better algorithm was developed \sidecite{fearnley_faster_2022}, which mostly relies on a faster way of finding a progress point in the three-dimensional case, which they call the \emph{inner algorithm}. An \emph{outer algorithm} then repeatedly applies the inner algorithm on 3-dimensional instances. Overall this approach achieves a runtime of $\BigO{\log^{2\ceil{\frac{d}{3}}}{N}}$ for $L = [N]^d$. This is, to date, the best upper bound for solving \Tarski\ instances.
...
A further notable collapse is the result $\PLS \medcap \PPAD = \EOPL$, which was only recently shown in \sidecite{goos_further_2022}. This, of course, means that, in particular, \Tarski\ lies in \EOPL. A question that then arises, and which this thesis will attempt to answer, is whether we can construct an explicit reduction of \Tarski\ to \EndOfPotentialLine.
