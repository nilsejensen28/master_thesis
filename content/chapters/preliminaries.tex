...
The \Tarski-problem originates from Tarski's fixed point Theorem, introduced in \sidecite{tarski_lattice-theoretical_1955}. The Theorem states the following:
...
\end{proof}
This means that in this thesis, we can consider the simplified version of the \Tarski\ problem\marginnote{Also note that given a circuit that computes $f$ we can construct a circuit with at most double the size which computes $\ftilde$.}, where for every $x \in L$, we have $\norminf{x - f(x)} \leq 1$, which we will implicitly assume from now on.
