...
Here, so-called \define{search problems} come into play.
...
As we will see, the problems we will study are given by questions of the form ``find an $x \in S$ such that $f(x)$ has some property''. A question which arise, that answer next, is how we represent the input, that is, the set $S$ and the function $f$. We start by describing how we represent sets.
...
These boolean circuits represent a directed graph with maximal in and out-degree one by having an edge from $x$ to $y$ if and only if $S(x) = y$ and $P(y) =
    x$.
...
Next, we want to discuss the complexity class \EOPL{} introduced in \sidecite{EOPL_introduction}. The existence results giving rise to \EOPL\ is:
