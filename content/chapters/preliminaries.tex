...
Finally there is one final way to obtain a \TFNP\ problem, which we want to highlight. We can add \define{violations} to the solution space. For instance if we are interested in finding the global minima of convex functions, we can construct a total search problem by:
\begin{enumerate}
...
\boxmarginnote{Here $S$ can be thought of giving the successor of a vertex, and $P$ as giving the predecessor of a vertex.}
...
\begin{theorem}[Tarski's fixed point Theorem]
    \boxmarginnote{This theorem is also known as the Knaster–Tarski Theorem in the litterature.}
